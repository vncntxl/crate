...
**Files:** `account.php`, `js/account.js`, `api/my_reviews.php`, `api/account_update.php`, `api/password_change.php` (also reuses `api/review_add.php` and `api/review_delete.php` from [07-album-detail-and-reviews.md](07-album-detail-and-reviews.md))
...
PHP already knows who's logged in, since it just ran `require_login()`,
so there's no reason to make a separate `fetch()` just to ask "who am
I?" before the form can render with real values in it.
...
Editing reuses `api/review_add.php` exactly as it is; it already knows
how to update an existing review by `user_id` and `album_id` (see
[07-album-detail-and-reviews.md](07-album-detail-and-reviews.md)).
