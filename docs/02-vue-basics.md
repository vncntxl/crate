...
The first line downloads Vue itself from a public server (a "CDN" -
content delivery network) — this is exactly like linking a font or an
icon library, except what we're linking is a JavaScript framework. There
is no `npm install`, no build step, no compiling. The browser just
...
files. Every page's JS file (`app.js`, `album.js`, `account.js`...) is a
completely separate, self-contained script. If two pages both need a
small piece (like the read-only star display), that piece is just
...
in the viva - a build tool would let us share code between files, but we
didn't need one for a project this size.
...
- `createApp({...})` builds a Vue **application** out of a plain
  ...
  (`methods`)."
- `.mount('#app')` is the step where Vue actually takes control. It finds
  the element with `id="app"` in the HTML (every page has exactly one -
  e.g. `<div id="app">` in `index.php`) and says "everything inside this
  element is now mine to manage." From that point on, Vue watches the
  data and automatically redraws any part of that HTML that depends on
  it, whenever it changes - you never write code that manually finds an
  element and changes its text or its class; you just change the data,
  and Vue does the redrawing.
...
Vue feel "alive" compared to plain HTML: you never say "now go update the
page" - you just update the data, and the page catches up on its own.
...
- **`:class="{ active: genre === activeGenre }"`** - class binding. Vue
  ...
  heart buttons visually show "this one is selected/favourited" without
  any manual `classList.add`/`remove` code.
- **`@click="filterByGenre(genre)"`** - the `@` is shorthand for
  `v-on`. It wires up an event listener, here calling a method when the
  button is clicked.
...
A **component** is a small, named, reusable chunk of template + logic.
Crate defines two:
...
- **`props`** are how data flows *into* a component from whoever uses it
  - like a function's parameters. `<star-rating :rating="4"></star-rating>`
  passes the number 4 in as the `rating` prop.
- This component is **read-only** - it just displays stars, it can't be
  clicked. It's used on the home page... actually it's used on the album
  page and account page to show an existing rating.
