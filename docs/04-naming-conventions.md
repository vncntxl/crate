...
Naming isn't random in this project. Each language and layer follows the
...
uses `camelCase` in PHP too, for two reasons. Modern PHP style guides
(PSR-12) allow either and increasingly favour camelCase. More
importantly for this project, the *same* piece of information often
travels between PHP and JS (an album id, for example), so keeping the
variable name spelled the same way in both places (`albumId` in JS,
`$albumId` in PHP) makes it easier to trace a value's journey through
the app when explaining it out loud.

**PHP function names are the one exception.** `is_logged_in()`,
`current_user()`, and `require_login()` use `snake_case`, because
they're modelled on PHP's own built-in functions (`password_hash()`,
`htmlspecialchars()`, `array_rand()`), which are all `snake_case`. Our
own helper functions blend in with the language's standard library
...
world" meets "database world," and pointing it out in the viva is
evidence of understanding *why* a convention exists, not just following
it blindly.

## Common name prefixes and patterns used throughout
...
- **`is`/`has` prefix for booleans**: `isLoggedIn`, `isFavourited`,
  ...
  type.
- **`my` prefix for "belongs to the current session user"**: `myRating`,
  `myReviewText`, `myReviewId` on the album page, versus plain `reviews`
  for the full public list. This distinguishes the form you're about to
  submit from everyone else's reviews you're just displaying.
- **`Stmt` suffix for a prepared statement object**, as opposed to the
  plain variable holding the *result* of running it. `$existingStmt` is
  the prepared query; `$existing` is the row it found, or `false` if
  none.
- **Function names are verbs; component and prop names are nouns.**
