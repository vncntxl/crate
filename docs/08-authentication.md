...
use Vue and `fetch()` like the rest of the app.
...
the user typed (`value="<?= e($name) ?>"`), so they don't have to
retype everything.
...
taken. That last check runs as a `SELECT` before the `INSERT`, so a
duplicate email shows a friendly message instead of a raw database
error from the `UNIQUE` constraint on `users.email`.

Registering logs you in immediately. There's no separate "verify your
email" step in this project, so there's no reason to send a new user
...
The error message is identical whether the email doesn't exist or the
password is wrong, deliberately. Showing a different message for each
("no account with that email" versus "wrong password") would let an
attacker use the login form to check which emails have accounts at all,
one guess at a time.
